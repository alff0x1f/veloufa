<?php
/**
 * Advanced Widget Pack - Random Posts
 */

class Advanced_Widget_Pack_Widget_RandomPosts extends WP_Widget {
	
	/* The plugins slug */
	const SLUG = 'awp_random_posts';
	...
	public function __construct(){
		...
		$widgetOptions = array(
			'classname' => self::SLUG,
			'description' => __('Displays a custom list of random posts including thumbnail images.', self::SLUG)
		);
		...
		/* Create the widget */
		$this->WP_Widget(self::SLUG, __('AWP - Random Posts', self::SLUG), $widgetOptions, $controlOptions);
	...
	public function form($instance) {
		...
		?>
            ...
            <fieldset class="widefat advancedwidgetpack-general">
                ...
                <legend><?php _e('Random Posts Settings', self::SLUG); ?></legend>
			...
			if($title) echo $before_title.$title.$after_title;	
				...
				<div class="textwidget">
					<div class="advancedwidgetpack" <?php echo $widgetwidth == '' ? '' : 'style="width:'.$widgetwidth.$width_by.' !important"';?>>
						<?php 
							$this->advanced_widget_pack->awp_get_posts('random', $cat, $post_type, $count, $chars, $thumbnail == true ? true : false);
						?>
						<div class="clear"></div>
					</div>
				</div>
